...
public class InsaneAI : Agent
{
    ...
    private int knifeAttackCount = 0;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (fov != null && wp.ammoCapacity > 0)
        {
            ...
            if (fov.visibleTargets.Count > 0)
            {
                ...
                {
                    if(Vector3.Distance(transform.localPosition, target.localPosition) <= 3.5f)
                    {
                        MoveTowardsTarget(firstTarget);
                        if (Vector3.Distance(transform.localPosition, firstTarget.localPosition) < 1.0f)
                        {
                            wp.KnifeZone();
                            knifeAttackCount++;

                            if (knifeAttackCount >= 3)
                            {
        ...
        if (fov != null && wp.ammoCapacity <= 0 && wp.remainingAmmo <= 0 && isInSide)
        {
            fov.FindVisibleTargets();
            if (fov.visibleTargets.Count > 0)
            {
